...
type VideoResponse = {
    ...
    metadata?: { result_url?: string; video_url?: string; url?: string } | null;
};
...
const VIDEO_TASK_TIMEOUT_MS = 30 * 60_000;
...
function videoResultUrl(payload: VideoResponse) {
    return [payload.video_url, payload.result_url, payload.url, payload.content?.video_url, payload.content?.url, payload.metadata?.video_url, payload.metadata?.result_url, payload.metadata?.url].find(
