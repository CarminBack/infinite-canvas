...
import { enforceFixedChannels, useConfigStore, type AiConfig, type WebdavSyncConfig } from "@/stores/use-config-store";
...
export async function importAppConfig(file: File) {
    ...
    useConfigStore.setState({ config: enforceFixedChannels(data.config), webdav: data.webdav });
