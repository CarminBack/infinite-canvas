...
type VideoSettingsPanelProps = {
    config: AiConfig;
    onConfigChange: (key: "vquality" | "size" | "videoSeconds" | "videoModeType" | "videoGenerateAudio" | "videoWatermark", value: string) => void;
    theme: CanvasTheme;
    showTitle?: boolean;
    className?: string;
};
            ...
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title="生成模式" color={theme.node.muted}>
                    <div className="grid grid-cols-2 gap-2.5">
                        {seedanceModeOptions.map((item) => {
                            const disabled = item.value !== "auto" && !capabilities.modes.includes(item.value);
                            return (
                                <OptionPill key={item.value} selected={(config.videoModeType || "auto") === item.value} disabled={disabled} theme={theme} onClick={() => onConfigChange("videoModeType", item.value)}>
                                    {item.label}
                                </OptionPill>
                            );
                        })}
                ...
                <SettingGroup title="分辨率" color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        {seedanceResolutionOptions
                            .filter((item) => !capabilities.resolution || item.value === capabilities.resolution)
                            .map((item) => (
                                <OptionPill key={item.value} selected={resolution === item.value} disabled={Boolean(capabilities.resolution)} theme={theme} onClick={() => onConfigChange("vquality", item.value)}>
                                    {item.label}
                                </OptionPill>
                            ))}
                    </div>
                ...
                <SettingGroup title="比例" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                        {seedanceRatioOptions
                            .filter((item) => capabilities.ratios.includes(item.value))
                            .map((item) => (
                                <button
                                    key={item.value}
                                    type="button"
                ...
                <SettingGroup title="时长" color={theme.node.muted}>
                    <div className="grid grid-cols-4 gap-2.5">
                        {seedanceDurationOptions
                            .filter((value) => value >= capabilities.minDuration && value <= capabilities.maxDuration)
                            .map((value) => (
                    ...
                    <NumberInput value={String(duration)} min={capabilities.minDuration} max={capabilities.maxDuration} theme={theme} onChange={(value) => onConfigChange("videoSeconds", value)} />
                    <div className="text-[11px] leading-4 opacity-55">
                        最多 {capabilities.maxImages} 图 / {capabilities.maxVideos} 视频 / {capabilities.maxAudios} 音频
                    </div>
                </SettingGroup>
...
export function videoResolutionLabel(value: string) {
    ...
    return normalized.toLowerCase() === "4k" ? "4K" : `${normalized}p`;
}
...
function NumberInput({ value, min, max, theme, onChange }: { value: string; min: number; max: number; theme: CanvasTheme; onChange: (value: string) => void }) {
    ...
    return (
        <input
            ...
            value={value}
            onChange={(event) => onChange(event.target.value)}
            onBlur={(event) => onChange(clampValue(event.target.value))}
            onMouseDown={(event) => event.stopPropagation()}
        />
    );
}
